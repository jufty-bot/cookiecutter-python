...
records template answers and the source revision in `.github/.copier-answers.yaml`, which
...
Commit the generated `.github/.copier-answers.yaml` file with the rest of the
project.
...
Do not create `.github/.copier-answers.yaml` manually in an existing project.
